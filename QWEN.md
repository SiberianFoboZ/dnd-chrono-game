...
│   │   ├── ElPage.vue         # 47 эпизодов + 58 иллюстраций (~728 строк)
...
## Временные файлы (`new_add_text/`)

Локальный gitignored каталог для черновиков рукописи **до** переноса в основной код. Структура с тремя подпапками по типу контента:

| Подкаталог     | Назначение                                                                                       |
| -------------- | ------------------------------------------------------------------------------------------------ |
| `texts/`       | Сырые тексты дневников/глав перед переносом в `.vue`-страницы. Форматы: `.txt`, `.md`, `.docx`.    |
| `images/`      | Картинки для иллюстраций перед копированием в `public/images/<slug>/`. Форматы: `.png`, `.jpg`, `.jpeg`. |
| `notes/`       | Черновики, заметки, планы. Форматы: `.md`, `.txt`.                                              |

Конвенция именования — префикс персонажа (`el-dnev.docx`, `artur-chapter-1.txt` и т.п.). Подробности — в `new_add_text/README.md`.
